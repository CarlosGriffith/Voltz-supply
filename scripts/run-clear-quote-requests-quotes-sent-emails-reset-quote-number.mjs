/**
 * Clears pos_quote_requests, pos_quotes, pos_sent_emails; resets quote doc counter (next QT-1000001).
 *
 *   npm run db:clear-website-quotes-sent-emails-reset-quote
 *
 * Requires AIVEN_MYSQL_PASSWORD (and host/user/database per .env).
 */
...
const sqlPath = path.join(__dirname, 'clear-quote-requests-quotes-sent-emails-reset-quote-number.sql');
...
await conn.end();
console.log('Done — quote requests, quotes, sent emails cleared; next quote number QT-1000001.');
