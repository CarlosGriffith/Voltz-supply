/**
 * Adds pos_quote_requests.customer_id if missing (scripts/add-pos-quote-requests-customer-id.sql).
 * Uses .env AIVEN_MYSQL_* like db:bootstrap:aiven.
 *
 *   npm run db:add-quote-requests-customer-id-column
 */
...
const sqlPath = path.join(__dirname, 'add-pos-quote-requests-customer-id.sql');
...
try {
  ...
  console.log('OK — customer_id column added.');
} catch (e) {
  const msg = e?.message || String(e);
  if (/Duplicate column name/i.test(msg)) {
    console.log('OK — customer_id already exists (no change).');
  } else {
    console.error(msg);
    process.exitCode = 1;
  }
