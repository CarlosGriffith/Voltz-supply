/**
 * Adds pos_quote_requests.email_sent_at if missing (scripts/add-pos-quote-requests-email-sent-at.sql).
 * Uses .env AIVEN_MYSQL_* like db:bootstrap:aiven.
 *
 *   npm run db:add-email-sent-column
 */
...
const sqlPath = path.join(__dirname, 'add-pos-quote-requests-email-sent-at.sql');
...
try {
  ...
  console.log('OK — email_sent_at column added.');
} catch (e) {
  const msg = e?.message || String(e);
  if (/Duplicate column name/i.test(msg)) {
    console.log('OK — email_sent_at already exists (no change).');
  } else {
    console.error(msg);
    process.exitCode = 1;
  }
